...
if (strlen($_GET['CPU']>0)){
  $sql="SELECT * from kyc where id='".$_GET['id']."'";

}
else {
  $sql="SELECT * from kyc where id='90'";
...
//error_log("image1:".$data);
//$pdf->SetY($y+34);
//$pdf->SetX(65);


//error_log("image:".$data);
...
//$pdf->Image($file, 40, 190, 937,150, 'PNG', '', '', true, 150, '', false, $mask,0,false,true);
//$pdf->Image($file, 0, 200, '', '', 'PNG', '', '', false, '', '', false, false, 0, false, false,false);
...
<p>(CPU):  N.<b>".$contractor['CPU']." </b> del <b>".date('d/m/Y',strtotime($contractor['contract_date']) )."</p>";
// writeHTMLCell($w, $h, $x, $y, $html='', $border=0, $ln=0, $fill=0, $reseth=true, $align='', $autopadding=true)
...
$pdf->MultiRow("Data e Luogo di nascita", date('d/m/Y',strtotime($contractor['dob']))." a: " .$contractor['birth_town'] ." - " .ucfirst($cl[$contractor['birth_country']]) ,0,1);
$pdf->MultiRow("Residenza Anagrafica",$contractor['address_resi']." - ". $contractor['town_resi'] ."<br/>".ucfirst($cl[$contractor['resi_country']]),0,1,2);
if ($contractor_data['check_residence'])
$txt="Domicilio e residenza coincidono";
else
$txt=$contractor['address_domi']." - ". $contractor['town_domi'] ."<br/>".ucfirst($cl[$contractor['domi_country']]);
...
$txt=$contractor['profession'] . " - " . ucfirst($cl[$contractor['main_activity_country']]);
...
$pdf->writeHTMLCell(80,80, 118, 171, $sign, 0, 0, 1, true, 'L', true);
  ...
  $pdf->SetFillColor(255, 255, 255);
  $pdf->SetFont('times', '', 10);
  $txt="Il Sottoscritto dichiara che agisce con delega de " . $contractor['other_name'];
  if($contractor['act_for_other']==2)
  $txt="Il Sottoscritto dichiara che agisce con delega dei seguenti n.".count($other)." titolari effettivi: ";
  if (count($other)==1) $txt="Il Sottoscritto dichiara che agisce con delega del seguente titolare effettivo: ";
  ...
  foreach ($other as $oth){
    if ($i>0){
      ...
      $pdf->SetX(5);
      $txt=strtoupper("Dati identificativi dei Titolari Effettivi")."<sup>2</sup>";
      $pdf->writeHTMLCell(190, 1, 0, '', html_entity_decode($txt), 0, 0, 1, true, 'J', true);
...
//      $pdf->Image($file, 45, 164, 937, 150, '', '', '', true, 300, '', false, false, 0, false, false, true);
    ...
    $pdf->MultiRow("Data e Luogo di nascita", date('d/m/Y',strtotime($other[$i]['dob']))." a: " .$other[$i]['birth_town'] ." - " .ucfirst($cl[$other[$i]['birth_country']]) ,0,1);
    $pdf->MultiRow("Residenza Anagrafica",$other[$i]['address_resi']." - ". $other[$i]['town_resi'] ."<br/>".ucfirst($cl[$other[$i]['resi_country']]),0,1,2);
    if ($contractor_data['check_residence'])
    $txt="Domicilio e residenza coincidono";
    else
    $txt=$other[$i]['address_domi']." - ". $other[$i]['town_domi'] ."<br/>".ucfirst($cl[$other[$i]['domi_country']]);
    ...
    $pdf->MultiRow("Recapiti Telefonici", $txt,0,1);
    $txt=$other[$i]['profession'] . " - " . ucfirst($cl[$other[$i]['main_activity_country']]);
    $pdf->MultiRow("Tipo di Professione e zona geografica nella quale si svolge prevalentemente", $txt,0,1,2);
...
$txt= money_format('%.2n', $contractor['contract_value']) ;
...
$txt=ucfirst($cl[$contract['activity_country']]);
...
echo $file;
file_put_contents('kyc'.$contract['contract_id'].'-'.$contract['agency_id'].'.pdf',$file);
$dati=array('mailto'=>$contractor['email'],'CPU'=>$contract['CPU'],'fullname'=>$contractor['fullname'],
'file'=>'kyc'.$contract['contract_id'].'-'.$contract['agency_id'].'.pdf');
email_attach($dati);
